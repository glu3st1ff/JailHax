...
-- [[ Server List via Roblox API (replacing api.jailhax.lol) ]]
local function FetchServers(placeId, preferSmall)
    local result = { Data = {}, Success = true }
    local cursor = ""
    local pages = 0
    while true do
        ...
        for _, s in ipairs(res.data) do
            if type(s) == "table" and s.id and s.maxPlayers and s.playing then
                if (not preferSmall) or (s.playing < (s.maxPlayers - 2)) then
                    table.insert(result.Data, s)
                end
            end
        end
        cursor = res.nextPageCursor or ""
        pages = pages + 1
        if cursor == "" or pages >= 5 then break end
    end
    return result
end
...
local function GetDirectory()
	local Directory = "Jailbreak-AutoFarm"
...
local function ServerHop()
	...
	-- Removed remote download; optional self-host below.
if NO_RE_EXECUTE then
		...
		Queue = [[
			...
			if not success then
				loadstring(-- removed remote download)()	
			end
		]]
	end

	-- Optional: set getgenv().PatchedScriptURL to host this script yourself
	if getgenv().PatchedScriptURL then
		Queue = [[ while not game:IsLoaded() do task.wait() end; loadstring(game:HttpGet("]] .. getgenv().PatchedScriptURL .. [["))() ]]
	else
		Queue = Queue .. [[
			local ok, src = pcall(readfile, "]] .. ScriptFile .. [[")
	...
	while not Success do
		pcall(function()
			...
			repeat
				Servers = HttpService:JSONDecode((game:HttpGet("ROBLOX_API_SERVERS" .. ((Settings.SmallServers and "") or "?type=big"))))
...
-- [[ Minimal UI (headless) to replace dead jailhax.lol GUI ]]
local Library = (function()
    local lib = {}
    function lib:AddWindow(title, opts)
        local window = {}
        function window:AddTab(name)
            ...
            function tab:AddSwitch(name, callback)
                local state = false
                local obj = {}
                function obj:Set(v)
            ...
            function tab:AddSlider(name, callback)
                local val = 0
                local obj = {}
                function obj:Set(v)
    ...
    function lib:FormatWindows() end
    return lib
end)()

-- Library replaced with Minimal UI

do
	local Window = Library:AddWindow("Jailbreak Auto Farm (Patched)", {
	...
	do
		...
		MainTab:AddButton("Copy Discord Invite", function()
			StarterGui:SetCore("SendNotification", {
				Title = "Discord";
				Text = "Discord link removed.";
				Duration = 5;
			})

			-- removed discord invite copy
		end)
			...
			["embeds"] = {
				{
					...
					["color"] = 16711680,
					["thumbnail"] = {
						["url"] = "https://i.imgur.com/5QZ4QhR.png"
					},
					["footer"] = {
						["text"] = "Jailbreak Auto Farm"
					},
					["timestamp"] = ClockTime
				}
			},
